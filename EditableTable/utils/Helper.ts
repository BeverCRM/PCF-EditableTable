...
export function setDateTime(localDate: Date) {
  ...
  lastSundayOfMarch = new Date(
    lastSundayOfMarch.getFullYear(),
    lastSundayOfMarch.getMonth(),
    lastSundayOfMarch.getDate(),
    2, 0, 0);
  let lastSundayOfOctober = getLastSunday(utcDate.getFullYear(), 10);
  lastSundayOfOctober = new Date(
    lastSundayOfOctober.getFullYear(),
    lastSundayOfOctober.getMonth(),
    lastSundayOfOctober.getDate(),
    2, 0, 0);

  if (utcDate.getTime() >= lastSundayOfMarch.getTime() &&
      utcDate.getTime() < lastSundayOfOctober.getTime()) {
    additionalHour = 1;
  }
